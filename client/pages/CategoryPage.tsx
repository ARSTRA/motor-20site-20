...
export default function CategoryPage() {
  ...
  const fetchCarsByCategory = async (categorySlug: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/cars?category=${categorySlug}&sortBy=${sortField}&sortOrder=${sortOrder}&limit=20`,
      );
      const data = await response.json();
      setCars(data.cars);
    } catch (error) {
  ...
  const VehicleCard = ({
    ...
    <Card
      className={`group overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-gradient-to-br from-white to-gray-50 rounded-3xl border-2 border-transparent hover:border-ocean-200 ${isListView ? "flex" : ""}`}
    >
      <div className={`relative ${isListView ? "w-96 flex-shrink-0" : ""}`}>
          ...
          <div
            className={`absolute inset-0 ${
              car.category === "luxury-suv"
                ? "bg-gradient-to-br from-ocean-500/30 via-forest-500/20 to-sunset-500/20"
                : car.category === "sports-car"
                  ? "bg-gradient-to-br from-sunset-500/30 via-gold-500/20 to-ocean-500/20"
                  : car.category === "luxury-sedan"
                    ? "bg-gradient-to-br from-forest-500/30 via-ocean-500/20 to-gold-500/20"
                    : car.category === "electric"
                      ? "bg-gradient-to-br from-gold-500/30 via-forest-500/20 to-ocean-500/20"
        ...
        <div className="absolute top-6 left-6 flex gap-3">
          ...
          <Badge
            variant="secondary"
            className="bg-gradient-to-r from-forest-100 to-forest-200 text-forest-700 font-bold shadow-lg"
          >
      ...
      <CardContent className={`${isListView ? "flex-1" : ""} p-8`}>
        <div className="flex items-center justify-between mb-6">
          <Badge
            variant="secondary"
            className="bg-gradient-to-r from-forest-100 to-forest-200 text-forest-700 font-bold text-sm px-4 py-2"
          >
        ...
        <div
          className={`grid ${isListView ? "grid-cols-6" : "grid-cols-3"} gap-4 mb-8`}
        >
          <div className="bg-gradient-to-br from-ocean-50 to-ocean-100 p-4 rounded-2xl text-center hover:scale-105 transition-transform">
            ...
            <span className="text-xs text-ocean-600 block font-medium">
              Year
            </span>
          ...
          <div className="bg-gradient-to-br from-forest-50 to-forest-100 p-4 rounded-2xl text-center hover:scale-105 transition-transform">
            ...
            <span className="text-xs text-forest-600 block font-medium">
              Mileage
            </span>
          ...
          <div className="bg-gradient-to-br from-sunset-50 to-sunset-100 p-4 rounded-2xl text-center hover:scale-105 transition-transform">
            ...
            <span className="text-xs text-sunset-600 block font-medium">
              Fuel
            </span>
          ...
          {isListView && (
            <>
              <div className="bg-gradient-to-br from-gold-50 to-gold-100 p-4 rounded-2xl text-center hover:scale-105 transition-transform">
                <span className="text-xs text-gold-600 block font-medium">
                  Power
                </span>
        ...
        <div
          className={`flex ${isListView ? "flex-col xl:flex-row" : "flex-col sm:flex-row"} items-start justify-between gap-6`}
        >
          ...
          <div className="text-center">
            ...
            <p className="text-xl lg:text-2xl text-gray-100 mb-12 max-w-4xl mx-auto leading-relaxed">
              {currentCategory?.description ||
                "Discover our handpicked selection of premium vehicles, each representing the pinnacle of automotive excellence and luxury."}
            </p>
              ...
              <TabsContent key={cat.id} value={cat.slug} className="mt-8">
                <div className="bg-white rounded-3xl shadow-2xl p-8 border-2 border-gray-100">
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <Badge
                      variant="outline"
                      className="text-ocean-600 border-ocean-300 px-4 py-2 text-lg"
                    >
                  ...
                  {loading ? (
                    ...
                    >
                      {[...Array(6)].map((_, i) => (
                        <div
                          key={i}
                          className="bg-gray-200 animate-pulse rounded-3xl h-96"
                        ></div>
                      ))}
                    </div>
                  ) : cars.length === 0 ? (
                    <div className="text-center py-16">
                      ...
                      <p className="text-gray-600 mb-8 text-lg">
                        Check back soon for new additions to our{" "}
                        {cat.name.toLowerCase()} collection
                      </p>
                      <Link to="/inventory">
      ...
      <section className="py-20 bg-gradient-to-br from-ocean-600 via-forest-600 to-sunset-600 text-white relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 text-center relative z-10">
          ...
          <p className="text-xl lg:text-2xl mb-12 leading-relaxed max-w-3xl mx-auto text-gray-100">
            Our luxury automotive specialists are ready to help you experience
            the vehicle of your dreams. Schedule a private viewing or get
