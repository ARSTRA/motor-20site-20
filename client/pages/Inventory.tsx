...
export default function Inventory() {
  ...
  useEffect(() => {
    fetchCars();
  }, [searchTerm, selectedCategory, selectedBrand, priceRange, yearRange, selectedFuel, sortBy, sortOrder, showFeaturedOnly, currentPage]);
  ...
  const FilterSidebar = ({ mobile = false }) => (
    <div className={`space-y-6 ${mobile ? 'p-4' : ''}`}>
      {/* Search */}
      <div>
        <label className="block text-sm font-bold text-gray-700 mb-3">Search Vehicles</label>
  ...
  const VehicleCard = ({ car, isListView = false }: { car: Car; isListView?: boolean }) => (
    <Card className={`group overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50 rounded-2xl border-2 border-transparent hover:border-ocean-200 ${isListView ? 'flex' : ''}`}>
      <div className={`relative ${isListView ? 'w-80 flex-shrink-0' : ''}`}>
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          ...
          <Badge variant="secondary" className="bg-gradient-to-r from-forest-100 to-forest-200 text-forest-700 font-semibold">
      ...
      <CardContent className={`${isListView ? 'flex-1' : ''} p-8`}>
        ...
        <div className={`grid ${isListView ? 'grid-cols-5' : 'grid-cols-3'} gap-4 mb-6`}>
        ...
        {isListView && (
          <div className="grid grid-cols-2 gap-4 mb-6 text-sm">
            <div><span className="font-bold">Color:</span> {car.specifications.color}</div>
            <div><span className="font-bold">Interior:</span> {car.specifications.interior}</div>
            <div><span className="font-bold">Drivetrain:</span> {car.specifications.drivetrain}</div>
            <div><span className="font-bold">Transmission:</span> {car.transmission}</div>
          </div>
        )}
        
        <div className={`flex ${isListView ? 'flex-col lg:flex-row' : 'flex-col sm:flex-row'} items-start justify-between gap-4`}>
          <div>
      ...
      <section className="bg-gradient-to-br from-ocean-600 via-forest-600 to-sunset-600 text-white py-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10">
          <div className="text-center">
            <Badge variant="outline" className="mb-6 text-gold-300 border-gold-300 px-4 py-2">
              Premium Collection
            </Badge>
            ...
            <p className="text-xl lg:text-2xl text-gray-100 mb-8 max-w-3xl mx-auto leading-relaxed">
              Discover our complete collection of premium vehicles. Each car is meticulously 
              inspected and comes with our guarantee of quality and excellence.
            </p>
            <div className="flex items-center justify-center gap-4 text-lg">
              <div className="flex items-center gap-2">
                <span className="text-4xl font-bold text-gold-300">{totalCount}</span>
                <span>Premium Vehicles</span>
              </div>
              <span className="text-gold-300">•</span>
              <div className="flex items-center gap-2">
                <span className="text-4xl font-bold text-gold-300">{categories.length}</span>
              ...
              <div className="flex items-center gap-4">
                ...
                <Sheet>
                  <SheetTrigger asChild>
                    <Button variant="outline" className="lg:hidden border-2 border-ocean-500 text-ocean-600">
            ...
            <div className="flex-1">
              {loading ? (
                ...
                <div className="text-center py-16">
                  ...
                  <Button onClick={clearFilters} className="bg-gradient-to-r from-ocean-500 to-forest-500 text-white">
                    Clear All Filters
                  </Button>
                </div>
              ) : (
      ...
      <section className="py-16 bg-gradient-to-r from-gray-50 to-ocean-50">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-gray-600 mb-8 text-lg">
            Our expert team is here to help you find exactly what you're looking for
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button variant="outline" className="border-2 border-sunset-500 text-sunset-600 hover:bg-sunset-500 hover:text-white font-bold px-8 py-3 rounded-xl">
              <Mail className="h-4 w-4 mr-2" />
              Email Us
            </Button>
            <Button variant="outline" className="border-2 border-forest-500 text-forest-600 hover:bg-forest-500 hover:text-white font-bold px-8 py-3 rounded-xl">
